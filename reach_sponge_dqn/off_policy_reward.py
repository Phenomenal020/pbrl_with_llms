...
class RewardModel:
    ...
    def __init__(self, #✅
            ...
            mb_size = 100,      # mini-batch size used during ablation. ✅#
    ...
    def add_data(self): # ✅
        file_paths = [
            ...
            "ablation/llm/trajectories/enhanced_reach_sponge3000.jsonl",
            "ablation/llm/trajectories/enhanced_reach_sponge3001.jsonl",
            "ablation/llm/trajectories/enhanced_reach_sponge3002.jsonl",
            "ablation/llm/trajectories/enhanced_reach_sponge3003.jsonl" ,
            "ablation/llm/trajectories/enhanced_reach_sponge3004.jsonl",
            "ablation/llm/trajectories/enhanced_reach_sponge3005.jsonl",
            "ablation/llm/trajectories/enhanced_reach_sponge3006.jsonl",
            "ablation/llm/trajectories/enhanced_reach_sponge3007.jsonl"
            "ablation/llm/trajectories/enhanced_reach_sponge3008.jsonl",
            "ablation/llm/trajectories/enhanced_reach_sponge3009.jsonl",
            "ablation/llm/trajectories/enhanced_reach_sponge3010.jsonl",
            "ablation/llm/trajectories/enhanced_reach_sponge3011.jsonl",
            "ablation/llm/trajectories/enhanced_reach_sponge3012.jsonl",
            "ablation/llm/trajectories/enhanced_reach_sponge3013.jsonl",
            "ablation/llm/trajectories/enhanced_reach_sponge3014.jsonl",
            "ablation/llm/trajectories/enhanced_reach_sponge3015.jsonl"
            ]  # For ablation
            ...
            try:
                with open(file_path, "r") as f:
                    ...
                    for line in f:
                        ...
                        self.imageobses.append(data["image"])
    ...
    def get_llm_labels(self, obs1, obs2, acts1, acts2, r1, r2, pred_labels, true_labels, dones1, dones2, truncated1, truncated2, collisions1, collisions2, successes1, successes2, sum_r1, sum_r2, sum_r1_pred, sum_r2_pred, use_rag=True):  # ✅
        ...
        if valid_indices:
            ...
            with jsonlines.open("enhanced_rl/db/preferences/preferences.jsonl", mode='a') as writer:
                for i in valid_indices:
                    ...
                    writer.write(entry)
                    print(f"Saved valid preference for index {origin_index + i} to enhanced_rl/db/preferences/preferences.jsonl")
        else:
    ...
    def sample_queries(self, samp_index=1):  # ✅
        ...
        if samp_index == 1:  # Use uniform sampling methods
            ...
            observations1, observations2, actions1, actions2, true_rewards1, true_rewards2, predicted_rewards1, predicted_rewards2, dones1, dones2, truncated1, truncated2, collisions1, collisions2, successes1, successes2 = self.disagreement_sampling()
    ...
    def pretrain(
        ...
        if max_idx == 0:
            print("[RewardModel.pretrain] Buffer is empty, nothing to pretrain.")
            return False
        ...
        print(f"[RewardModel.pretrain] {total} transitions → pretraining for {epochs} epochs")
        ...
        for epoch in range(epochs):
            ...
            print(f"[pretrain] Epoch {epoch+1}/{epochs} done, last loss = {loss.item():.6f}")

        print("[RewardModel.pretrain] Finished.")
        return True
...
class RunningMeanStd:
    def __init__(self,
                 ...
                 mode: str = 'common',
                 lr: float = 0.1,
                 save_path: str = "running_meanstd.jsonl"):
        self.eps      = epsilon
        self.mode     = mode
        self.lr       = lr
        self.save_path = "running_meanstd.jsonl"

        # default initial values
        self.mean  = float(mean)
        self.var   = float(std)   # note: this is variance, not std
        self.max   = float(mean)
        self.count = 0

        # if there's a save_path and the file exists, load the last line
        if save_path:
            obj = {}   # or obj = {'mean': self.mean, 'var': self.var, ...}
            with open(save_path, 'r') as f:
                for line in f:
                    try:
                        ...
                        continue
            # restore state
            self.mean  = obj.get('mean',  self.mean)
            self.var   = obj.get('var',   self.var)
            self.max   = obj.get('max',   self.max)
            self.count = obj.get('count', self.count)
            print(f"[RunningMeanStd] Loaded mean, var, max, count from {save_path}")
        else:
            print("[RunningMeanStd] No save_path avaialable.")

    def update(self, data_array: np.ndarray) -> None:
        batch_mean, batch_var = np.mean(data_array, axis=0), np.var(data_array, axis=0)
        batch_max  = np.max(data_array, axis=0)
        batch_count = data_array.shape[0]
        ...
        if self.mode == 'common':
            ...
            new_max  = self.max + (batch_max - self.max) / total_count
        else:
            new_mean = self.mean + delta * self.lr
            new_max  = self.max  + (batch_max - self.max) * self.lr

        # update variance via the “combined” formula
    ...
    def save(self) -> None:
        if not self.save_path:
            raise ValueError("No save_path specified")
...
if __name__ == "__main__":
    load_step = 32000 # FOR LOADING increment this after training
    ...
    loaded_files = reward_model.load(load_step=32000)
    if not loaded_files:
