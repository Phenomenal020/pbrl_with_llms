...
os.makedirs("pretrain/db/full_text/", exist_ok=True)
...
gpt_query_reach_sponge_no_rag = """Suppose you are an expert robot trajectory evaluator. You are provided with two trajectory segments from any two trajectories. Now you need to evaluate the quality of the robot's motion trajectory. The environment is a care bathing environment where a robot is expected to move towards a sponge inorder to grasp it. Consider priorities as (FIRST: success or collision (they are mutually exclusive); SECOND: progress towards the sponge (penalise unncessary turning); THIRD: truncation) in that order. BUT CONSIDER ALL and not exclusively one so your analysis is more robust.
...
def gpt_infer_no_rag(traj_str, traj_str2, prompt=gpt_query_reach_sponge_no_rag, index=1):
    ...
    with jsonlines.open("pretrain/db/full_text/full_text.jsonl", mode='a') as writer:
        writer.write({"full_text": full_text, "input_token_count": input_token_count, "output_token_count": output_token_count})
        print("Saved segment to pretrain/db/full_text/full_text.jsonl") 
...
gpt_query_reach_sponge_rag = """Suppose you are an expert robot trajectory evaluator. You are provided with two trajectory segments from any two trajectories AND expert-generated actions in a similar starting position. Now you need to evaluate the quality of the robot's motion trajectory. The environment is a care bathing environment where a robot is expected to move towards a sponge inorder to grasp it. Consider priorities as (FIRST: success or collision (they are mutually exclusive); SECOND: progress and how well it follows the expert's actions (assign equal weightage and penalise unncessary turning); THIRD: truncation) in that order. BUT CONSIDER ALL and not exclusively one so your analysis is more robust.
Note: The segments may start at different locations, so progress should not only consider the distance to the sponge but also how much the robot has moved towards the sponge relative to its starting position in the segment.
