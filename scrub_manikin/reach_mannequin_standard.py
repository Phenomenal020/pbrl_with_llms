...
os.makedirs("standard_rl/model/dqn", exist_ok=True)
os.makedirs("standard_rl/reward_model/dqn", exist_ok=True)
os.makedirs("standard_rl/replay_buffer/dqn", exist_ok=True)
...
if __name__ == "__main__":
    ...
    model_dir = "standard_rl/model/dqn"
    buffer_dir = "standard_rl/replay_buffer/dqn"
    ...
    latest_timestep = 0
    for f in glob.glob(os.path.join(model_dir, "standard_rl_reach_sponge_dqn*.zip")):
        try:
    ...
    for lts in range(latest_timestep, TOTAL_TIMESTEPS, SAVE_INTERVAL):
        ...
        checkpoint = os.path.join(model_dir, f"standard_rl_reach_sponge_dqn{timestep}.zip")
        buffer = os.path.join(buffer_dir, f"replay_buffer_{timestep}.npz")
        model.save(checkpoint)
        save_replay_buffer(model.replay_buffer, buffer)
        # model.replay_buffer.save(buffer, "standard_rl")
