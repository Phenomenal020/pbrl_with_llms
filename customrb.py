...
class DQNPBRLReplayBuffer(ReplayBuffer):
    ...
    def relabel_with_predictor(self, predictor, batch_size: int = 100):
        ...
        # For multiple environments, self.observations has shape:
        #  (self.buffer_size, self.n_envs, *self.obs_shape)
        # To process all samples across all environments, we flatten the first two dimensions.
        # Get the updated rewards
        # Then reshape to original shape so each environment maintains its data
        n_envs = self.n_envs
        flat_total_samples = total_samples * n_envs

        # Flatten observations and rewards for relabelling:
        flat_obs = self.observations[:total_samples].reshape(-1, *self.obs_shape)
        flat_rewards = self.rewards[:total_samples].reshape(-1)

        # Compute the number of batches (round up for any partial batch)
        total_iter = int(flat_total/batch_size)
        if self.pos > total_iter*batch_size:
            total_iter += 1

        # Process each batch
        for index in range(total_iter):
            last_index = (index+1)*batch_size # Get last index
            if last_index > self.pos:
                last_index = self.pos # Adjust for partial batch eg, 60 instead of 100

            start_index = index*batch_size

            # Slice the batch of observations from the flattened array.
            batch_obses = flat_obs[start_index:last_index]

            # Get predicted rewards from the reward model in a batched fashion.
            pred_rewards = predictor.r_hat_batch(batch_obses)

            # Replace the stored rewards in the flattened array.
            flat_rewards[start_index:last_index] = pred_rewards

        # After processing, reshape the flat_rewards back to the original shape
        # and update self.rewards for the valid samples.
        self.rewards[:total_samples] = flat_rewards.reshape(total_samples, n_envs)
