...
class ReachSponge(gym.Env):	
    ...
    def _get_reward(self, collision):
        ...
        if is_done or truncated:
            ...
            os.makedirs("./results/naive_rl/", exist_ok=True)

            # Append one JSON object per line to results.jsonl
            with jsonlines.open(f"./results/naive_rl/naive_rl_compare{self.port}.jsonl", mode="a") as writer:
                writer.write(result)
...
if __name__ == "__main__":
    ...
    manual_ports = [6000, 6005, 6010, 6015]
    ...
    for model_num in model_nums:
        model = PPO.load(f"{models_dir}/naive_reach_sponge_ppo{model_num}")
        print(f"Model {model_num} loaded.\n")

        obs = env.reset()
        for ep in range(1, num_episodes + 1):
          
            print(f"testing episode {ep} obs: {obs}")
            done = False
            total_reward = 0.0
...
# Test observation: [[-1.7691660e+00  8.7354556e-03 -5.0006413e-01  3.5795859e+02
#    2.6947891e+02  5.5601015e-03]]

# Test observation: [[-1.7691648e+00  8.7355301e-03 -5.0006402e-01  3.5795856e+02
#    2.6947916e+02  5.5671488e-03]]