...
class ReachSponge(gym.Env):	
    ...
    def __init__(self, use_graphics=False, dev=None, port: int = None):
        ...
        self.curr_pose = None # For evaluation.
        
        self.np_random, _ = gym.utils.seeding.np_random(42)
    ...
    def reset(self, seed=None, options=None):
        ...
        # For position
        # pose = self.np_random.choice(self.poses)
        pose = self.poses[self.n_episodes-1]
        print(f"Episode: {self.n_episodes} ***** pose: {pose}")
        x, y, z = pose[0][0], pose[0][1], pose[0][2]
        self.robot.SetPosition([x, y, z])
        # For rotation
    ...
    def _get_reward(self, collision):
        ...
        #     "distance-time_reward": dist_reward-time_reward,
        #     "returns": self.returns
        # })  
        # log_str = (
        #     "\n"
        #     "+-------------+---------------+--------------------+-------------+-------+-----------+--------------+-------------+\n"
        #     "| n_steps     | num_episodes  | num_episode_steps  | true reward | done  | truncated | n_collisions | n_successes |\n"
        #     "+-------------+---------------+--------------------+-------------+-------+-----------+--------------+-------------+\n"
        #     f"| {self.n_steps:<11} | {self.n_episodes:<13} | {self.n_eps_steps:<18} | {reward:<11} | {str(is_done):<5} | {str(truncated):<9} | {len(collision):<12} | {self.n_success:<11} |\n"
        #     "+-------------+---------------+--------------------+-------------+-------+-----------+--------------+-------------+\n"
        # )

        # train_logger.standard_logger.info(log_str)
        ...
        if is_done or truncated:
            result = {
                ...
                "num_episodes": self.n_episodes,
                "num_episodes_steps": self.n_eps_steps,
                "Current pose": self.curr_pose.tolist()
            }
            
            os.makedirs("./results/standard_rl/", exist_ok=True)

            # Append one JSON object per line to results.jsonl
            with jsonlines.open(f"./results/standard_rl/standard_rl_compare_{self.port}.jsonl", mode="a") as writer:
                writer.write(result)
...
if __name__ == "__main__":
    ...
    def make_env(port):
        env = ReachSponge(use_graphics=True, port=port)  
        env = Monitor(env, filename=f"standard_reach_sponge_monitor_{port}.csv")
        return env
    ...
    # latest_timestep = 0
    # checkpoint_files = glob.glob(os.path.join(models_dir, "standard_reach_sponge_ppo*.zip"))
    ...
    #         timestep = int(basename.replace("standard_reach_sponge_ppo", "").replace(".zip", ""))
    ...
    #     save_path = os.path.join(models_dir, f"standard_reach_sponge_ppo{cumulative_timestep}.zip")
    #     model.save(save_path)
    #     print(f"Model saved at timestep {cumulative_timestep}")
    
    
    # Evaluate
    # model_nums = [29000, 33000, 38000]
    model_nums = [38000]
    num_episodes = 20
    for model_num in model_nums:
        model = PPO.load(f"{models_dir}/standard_reach_sponge_ppo{model_num}")
