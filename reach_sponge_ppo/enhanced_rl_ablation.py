...
class ReachSponge(gym.Env):	
    ...
    def __init__(self, use_graphics=False, dev=None, port: int = None):
        ...
        self.file_db = f"./db/ablation/enhanced_reach_sponge{port}.jsonl"
        ...
        self.reward_model = RewardModel(ds=self.observation_space.shape[0], da=self.action_space.n)
        ...
        loaded_files = self.reward_model.load(model_dir="reward_model/enhanced_rl/ppo", load_step=100000)
        if not loaded_files:
    ...
    def step(self, action):
        ...
        predicted_reward = self.reward_model.r_hat(observation).detach().cpu().item()
    ...
    def process_episode(self, terminated, truncated):
        ...
        if terminated or truncated: 
            ...
            # 2. Instead of adding to a reward model, store in a jsonlines file
            def add_segment(start, end):
                # Create a dictionary with the segment data
                segment_data = {
    ...
    def _get_reward(self, collision, pred_reward):
        ...
        if is_done or truncated:
            result = {
                ...
                "n_successes": self.n_success,
                "returns": self.returns,
                "Current pose": self.current_pose.tolist()
            }
            
            os.makedirs("./results/enhanced_rl/", exist_ok=True)

            # Append one JSON object per line to results.jsonl
            with jsonlines.open(f"./results/enhanced_rl/enhanced_rl_compare{self.port}.jsonl", mode="a") as writer:
                writer.write(result)
...
if __name__ == "__main__":
    ...
    models_dir = "models/ablation/enhanced_rl/ppo"
    model_nums = [6000] # start, mid, finish
    num_trials = 10
    
    # Load the necessary model
    for model_num in model_nums:
        model = PPO.load(f"{models_dir}/enhanced_reach_sponge_ppo{model_num}")
        print(f"Model {model_num} loaded.\n")

        obs = env.reset()
        # for each episode
        for ep in range(1, num_trials + 1):
    ...
    # logdir = "logs/ablation/enhanced_rl/ppo"
    ...
    # else:
    #     print("No checkpoint found. Using the pretrained model.")
