...
def gen_net(in_size=1, out_size=1, H=128, n_layers=3, activation='tanh', use_batch_norm=False):  # ✅
    """
    Constructs a neural network with optional batch normalization.
...
class RewardModel:
    ...
    def __init__(self, #✅
            ...
            max_size=100,       # maximum number of training trajectories to keep (for FIFO logic) ✅  # Change to 12 to simulate (12x50) = 600 timesteps.
    ...
    def _add_data(self, obs, act, true_rew, pred_rew, done):  # ✅
        """
        Add data to the reward model's inputs buffer
        
        Parameters
        ----------
        obs : array_like
            A 2D array of shape `(T, ds)`.
        act : array_like
            A 2D array of shape `(T, da)`.
        true_rew : array_like
            A 2D array of shape `(T,)`.
        pred_rew : array_like
            A 2D array of shape `(T,)`.
        done : bool
        ...
        None
        """
        # For trajectory length 1, each sample is independent.
        # Combine observation and action if necessary.
        if self.traj_action:
            sa_t = np.concatenate([obs, act], axis=-1)
            sa_t = sa_t.reshape(1, self.da + self.ds)
        else:
            sa_t = obs.reshape(1, self.ds)
        
        # Process the reward and reshape it as a 1x1 target.
        action = np.array(act).reshape(1, 1)
        true_r = np.array(true_rew).reshape(1, 1)
        pred_r = np.array(pred_rew).reshape(1, 1)
        
        # If the inputs list is empty, add the first sample.
        init_data = len(self.obses) == 0

        # if the inputs list is empty
        if init_data:
            # Simply append the data to the array
            self.obses.append(sa_t)
            self.actions.append(action)
            self.true_rewards.append(true_r)
            self.pred_rewards.append(pred_r)
        # if this data is the last one in the trajectory   ``
        elif done:
            # Add the data to the last trajectory
            self.obses[-1] = np.concatenate([self.obses[-1], sa_t])
            self.actions[-1] = np.concatenate([self.actions[-1], action])
            self.true_rewards[-1] = np.concatenate([self.true_rewards[-1], true_r])
            self.pred_rewards[-1] = np.concatenate([self.pred_rewards[-1], pred_r])
            # FIFO - if obses is full, remove the first trajectory and add a new one for future appendage
            if len(self.obses) > self.max_size:
                self.obses = self.obses[1:]
                self.actions = self.actions[1:]
                self.true_rewards = self.true_rewards[1:]
                self.pred_rewards = self.pred_rewards[1:]
            self.obses.append([])
            self.actions.append([])
            self.true_rewards.append([])
            self.pred_rewards.append([])
        # if the buffer is not empty and this data is not the last one in the trajectory
        else:
            # Check if the last trajectory is empty
            if len(self.obses[-1]) == 0:
                # if so, append to the last element
                self.obses[-1] = sa_t
                self.actions[-1] = action
                self.true_rewards[-1] = true_r
                self.pred_rewards[-1] = pred_r
            else:
                # else, concatenate the data to the last trajectory
                self.obses[-1] = np.concatenate([self.obses[-1], sa_t])
                self.actions[-1] = np.concatenate([self.actions[-1], action])
                self.true_rewards[-1] = np.concatenate([self.true_rewards[-1], true_r])
                self.pred_rewards[-1] = np.concatenate([self.pred_rewards[-1], pred_r])
    ...
    def add_data(self, obs, act, true_rew, pred_rew, terminated, truncated): # ✅
        """
        Add a single step's data to the reward model's buffers.

        Parameters
        ----------
        obs : array_like
            A 1D or 2D array representing the observation at the current step.
        act : array_like
            A 1D or 2D array representing the action taken at the current step.
        true_rew : array_like
            The true reward for the current step.
        pred_rew : torch.Tensor or array_like
            The predicted reward for the current step.
        terminated : bool
            Whether the episode has terminated.
        truncated : bool
        ...
        print("Adding data to reward model...")
        data_logger.info("Adding data to reward model...")

        # Determine if the current step is the final step in the episode.
        done = terminated or truncated

        # Convert any scalar observations or actions into arrays
        obs = np.array([obs]) if np.isscalar(obs) else np.asarray(obs)
        act = np.array([act]) if np.isscalar(act) else np.asarray(act)
        
        # Handle trajectory action representation
        if self.traj_action:
            sa_t = np.concatenate([obs, act], axis=-1).reshape(1, self.da + self.ds)
        else:
            sa_t = obs.reshape(1, self.ds)

        # Process the action and true reward as 1x1 NumPy arrays
        action_arr = np.array(act).reshape(1, 1)
        true_r = np.array(true_rew).reshape(1, 1)

        # Ensure pred_rew remains a tensor, convert only when storing if necessary
        if isinstance(pred_rew, np.ndarray):
            pred_r = pred_rew.reshape(1, 1)  # Already NumPy
        else:
            pred_r = pred_rew.detach().cpu().numpy().reshape(1, 1)  # Convert tensor to NumPy

        # If this is the first step, initialise the buffers with the current step.
        if len(self.obses) == 0:
            self.obses.append(sa_t)
            ...
            # if the next step is the first step of a new trajectory
            if len(self.obses[-1]) == 0:
                self.obses[-1] = sa_t
                self.actions[-1] = action_arr
                self.true_rewards[-1] = true_r
                self.pred_rewards[-1] = pred_r
            else:
                # Append step to the latest trajectory
                self.obses[-1] = np.concatenate([self.obses[-1], sa_t])
                self.actions[-1] = np.concatenate([self.actions[-1], action_arr])
                self.true_rewards[-1] = np.concatenate([self.true_rewards[-1], true_r])
                self.pred_rewards[-1] = np.concatenate([self.pred_rewards[-1], pred_r])

        # If episode ends, start a new trajectory
        if done:
            # FIFO
            if len(self.obses) >= self.max_size:
                self.obses.pop(0) # Remove the oldest trajectory -
                self.actions.pop(0)
                self.true_rewards.pop(0)
                self.pred_rewards.pop(0)
            
            self.obses.append([]) # Append new empty trajectory
            self.actions.append([])
            self.true_rewards.append([])
            self.pred_rewards.append([])

            data_logger.info("Episode completed.")
            data_logger.info(f"Obses: {np.array_str(np.array(self.obses, dtype=object))}")
            data_logger.info(f"Actions: {np.array_str(np.array(self.actions, dtype=object))}")
            data_logger.info(f"True Rewards: {np.array_str(np.array(self.true_rewards, dtype=object))}")
            data_logger.info(f"Predicted Rewards: {np.array_str(np.array(self.pred_rewards, dtype=object))}")
            data_logger.info(f"Episode length: {len(self.obses[-1])}")
    ...
    def save(self, model_dir, step):  # ✅
        ...
        # Save each ensemble member's state dictionary
        for member in range(self.de):
            torch.save(
    ...
    # Load the model's state dictionary from disc - before rl_model.load()
    ...
    # Get queries  from the temp arrays for labelling (for the reward model)
    def _get_queries(self, mb_size=20): # ✅

        print("Getting queries...")

        # Ensure there is at least one complete trajectory
        if len(self.obses) == 0 or len(self.obses[0]) == 0:
            raise ValueError("No trajectory data available.")
        
        # Get the full trajectory length from the first trajectory
        full_traj_length = len(self.obses[0])
        
        # Exclude the last trajectory if it is incomplete.
        num_trajs = len(self.obses)
        if len(self.obses[-1]) < full_traj_length:
            num_trajs -= 1
        
        # Convert the first num_trajs trajectories into NumPy arrays.
        train_obses = np.array(self.obses[:num_trajs])      # shape: [num_trajs, full_traj_length, feature_dim]
        train_actions = np.array(self.actions[:num_trajs])    # shape: [num_trajs, full_traj_length, 1]
        train_true_rewards = np.array(self.true_rewards[:num_trajs])   # shape: [num_trajs, full_traj_length, 1]
        train_pred_rewards = np.array(self.pred_rewards[:num_trajs])   # shape: [num_trajs, full_traj_length, 1]
        
        # Randomly sample trajectories for both query sets - 1 & 2
        batch_idx_1 = np.random.choice(num_trajs, size=mb_size, replace=True)
        batch_idx_2 = np.random.choice(num_trajs, size=mb_size, replace=True)
        
        obs1 = train_obses[batch_idx_1]  # shape: [mb_size, full_traj_length, feature_dim]
        obs2 = train_obses[batch_idx_2]
        acts1 = train_actions[batch_idx_1]  # shape: [mb_size, full_traj_length, 1]
        acts2 = train_actions[batch_idx_2]
        true_r1 = train_true_rewards[batch_idx_1]  # shape: [mb_size, full_traj_length, 1]
        true_r2 = train_true_rewards[batch_idx_2]
        pred_r1 = train_pred_rewards[batch_idx_1]  # shape: [mb_size, full_traj_length, 1]
        pred_r2 = train_pred_rewards[batch_idx_2]
        
        # Determine random start indices for the window extraction for each query.
        # Note: full_traj_length should be larger than self.size_segment.
        start_indices_1 = np.random.randint(0, full_traj_length - self.size_segment + 1, size=mb_size)
        start_indices_2 = np.random.randint(0, full_traj_length - self.size_segment + 1, size=mb_size)
        
        # Extract the window (segment) from each trajectory.
        obs1 = np.array([traj[start:start+self.size_segment] for traj, start in zip(obs1, start_indices_1)])
        obs2 = np.array([traj[start:start+self.size_segment] for traj, start in zip(obs2, start_indices_2)])
        acts1 = np.array([traj[start:start+self.size_segment] for traj, start in zip(acts1, start_indices_1)])
        acts2 = np.array([traj[start:start+self.size_segment] for traj, start in zip(acts2, start_indices_2)])
        true_r1 = np.array([traj[start:start+self.size_segment] for traj, start in zip(true_r1, start_indices_1)])
        true_r2 = np.array([traj[start:start+self.size_segment] for traj, start in zip(true_r2, start_indices_2)])
        pred_r1 = np.array([traj[start:start+self.size_segment] for traj, start in zip(pred_r1, start_indices_1)])
        pred_r2 = np.array([traj[start:start+self.size_segment] for traj, start in zip(pred_r2, start_indices_2)])

        print("Got queries successfully...")
        
        return obs1, obs2, acts1, acts2, true_r1, true_r2, pred_r1, pred_r2
    ...
    def get_queries(self, mb_size=20): # ✅
        ...
        # Convert lists of trajectories into NumPy arrays
        ...
        batch_idx_1 = np.random.choice(len(self.obses), size=mb_size, replace=False)
        batch_idx_2 = np.random.choice(len(self.obses), size=mb_size, replace=False)
        
        # Retrieve full trajectories for both query sets
        obs1 = train_obses[batch_idx_1]
        obs2 = train_obses[batch_idx_2]
        acts1 = train_actions[batch_idx_1]
        acts2 = train_actions[batch_idx_2]
        true_r1 = train_true_rewards[batch_idx_1]
        true_r2 = train_true_rewards[batch_idx_2]
        pred_r1 = train_pred_rewards[batch_idx_1]
        pred_r2 = train_pred_rewards[batch_idx_2]
    ...
    def _put_queries(self, sa_t_1, sa_t_2, pred_labels, true_labels, llm_labels, fake_traj=False): # ✅
        ...
        assert sa_t_1.shape[0] == sa_t_2.shape[0] == pred_labels.shape[0] == true_labels.shape[0] == llm_labels.shape[0], "All inputs must have the same number of samples"

        # if rewards are provided, verify that they have the same number of samples.
        if r_t_1 is not None and r_t_2 is not None:
            assert r_t_1.shape[0] == num_samples and r_t_2.shape[0] == num_samples, "Reward arrays must match the number of samples"
        ...
        if next_index >= self.capacity:
            ...
            np.copyto(self.pred_labels_buffer[self.buffer_index:], labels[:remaining_space])
            np.copyto(self.llm_labels_buffer[self.buffer_index:], gt_labels[:remaining_space])
            self.fake_flag[self.buffer_index:] = fake_traj if np.isscalar(fake_traj) else fake_traj[:remaining_space]

            # If rewards are provided, insert them as well.
            if r_t_1 is not None and r_t_2 is not None:
                np.copyto(self.buffer_r1[self.buffer_index:], r_t_1[:remaining_space])
                np.copyto(self.buffer_r2[self.buffer_index:], r_t_2[:remaining_space])
            ...
            if remaining_samples > 0:
                ...
                np.copyto(self.pred_labels_buffer[:remaining_samples], labels[remaining_space:])
                np.copyto(self.llm_labels_buffer[:remaining_samples], gt_labels[remaining_space:])
                self.fake_flag[:remaining_samples] = fake_traj if np.isscalar(fake_traj) else fake_traj[remaining_space:]
                if r_t_1 is not None and r_t_2 is not None:
                    np.copyto(self.buffer_r1[:remaining_samples], r_t_1[remaining_space:])
                    np.copyto(self.buffer_r2[:remaining_samples], r_t_2[remaining_space:])
            ...
            np.copyto(self.pred_labels_buffer[self.buffer_index:next_index], labels)
            np.copyto(self.llm_labels_buffer[self.buffer_index:next_index], gt_labels)
    ...
    def get_label(self, obs1, obs2, acts1, acts2, true_r1, true_r2, pred_r1, pred_r2): # ✅
        """
        ...
        llm_labels = self.get_llm_labels(obs1, obs2, acts1_pred, acts2_pred, pred_r1_filt, pred_r2_filt, pred_labels, true_labels)  
    ...
    def get_llm_labels(self, obs1, obs2, acts1, acts2, r1, r2, pred_labels, true_labels): # ✅
        """
    ...
    def uniform_sampling(self):  # ✅
        ...
        observations1, observations2, actions1, actions2, true_rewards1, true_rewards2, predicted_rewards1, predicted_rewards2 = self.get_queries(mb_size=self.mb_size)
        ...
        obs1, obs2, true_r1, true_r2, predicted_r1, predicted_r2, true_labels, predicted_labels, llm_labels = self.get_label(observations1, observations2, actions1, actions2, true_rewards1, true_rewards2, predicted_rewards1, predicted_rewards2)
